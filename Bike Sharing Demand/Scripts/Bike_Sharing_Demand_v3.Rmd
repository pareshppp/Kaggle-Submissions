...
```{r convert_factor}
train.data[c(2:3, 11:15, 17)] <- 
    lapply(train.data[c(2:3, 11:15, 17)], factor)
```

We know a month has 31 days, so lets add the missing levels (20 to 31)
...
train.data$month.day <- addLevel(train.data$month.day, newlevel = c(20:31))
```
...
train.data %>% 
    ...
    distinct(season, weather,user.count) %>%
    ggplot(mapping = aes(season, user.count, fill = weather)) +
...
train.data %>%
    select(week.day, day.hour, count) %>%
    group_by(week.day, day.hour) %>%
    mutate(user.count = sum(count)) %>%
    distinct(week.day, day.hour, user.count) %>%
    ggplot(mapping = aes(x = week.day, y = user.count/1000, 
                         fill = day.hour)) +
...
train.data %>%
    select(temp, atemp, humidity, windspeed, temp.div, 
           casual, registered, count) %>%
    distinct() %>% cor() %>% round(digits = 1) %>%
    ggcorrplot(method = 'circle', 
               type = 'full', hc.order = T, lab = T, lab_size = 4, 
               ggtheme = theme_bw, 
...
```{r features_2}
feature.list.casual2 <- c(2, 5:6, 11:15, 17)
feature.list.reg2 <- c(2:3, 5:6, 11:12, 14:15, 17)
feature.list.count2 <- c(2, 5:6, 11:12, 14:15, 17)
```
...
test.data[c(2:3, 8:12, 14)] <-
    lapply(test.data[c(2:3, 8:12, 14)], factor)
```

Adding missing levels

```{r add_levels_test}
test.data$month.day <- addLevel(test.data$month.day, newlevel = c(1:19))
# test.data$month.day <- factor(test.data$month.day, 
#                               level = levels(train.data$month.day))
```
...
```{r features_test}
feature.list.casual.test <- c(2, 5:6, 8:12, 14)
feature.list.reg.test <- c(2:3, 5:6, 8:9, 11:12, 14)
feature.list.count.test <- c(2, 5:6, 8:9, 11:12, 14)
```
